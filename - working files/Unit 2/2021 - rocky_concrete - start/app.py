...
def product_details():
    if request.args.get('pd'):
        products = int(request.args.get('pd'))

        sql = """
                select prod_code, description, list_price, qty_on_hand
                from products
                where prod_code = ?;"""
        ...
        return render_template("product_details.html", title='Product Details', \
                               details=prods)
